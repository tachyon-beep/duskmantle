...
def evaluate_model(dataset_path: Path, model_path: Path) -> EvaluationMetrics:
    ...
    feature_rows, targets_list, request_ids = build_feature_matrix(records, model.feature_names)
    features = cast(FloatArray, np.asarray(feature_rows, dtype=np.float64))
    targets = cast(FloatArray, np.asarray(targets_list, dtype=np.float64))

    coeffs = cast(FloatArray, np.asarray(model.coefficients, dtype=np.float64))
    ...
    predictions = cast(FloatArray, features @ coeffs + model.intercept)
...
def _mean_ndcg(request_ids: Sequence[str], relevance: FloatArray, scores: FloatArray, *, k: int) -> float:
    ...
    for indices in groups.values():
        if not indices:
            continue
        rel = cast(FloatArray, relevance[indices])
        preds = cast(FloatArray, scores[indices])
        order = np.argsort(-preds)
        dcg = _dcg(cast(FloatArray, rel[order]), k)
        ideal = _dcg(cast(FloatArray, np.sort(rel)[::-1]), k)
        if ideal == 0:
            continue
...
def _dcg(relevances: FloatArray, k: int) -> float:
    """Compute discounted cumulative gain at rank ``k``."""
    k = min(k, len(relevances))
    if k <= 0:
        return 0.0
    gains = (np.power(2.0, relevances[:k]) - 1.0) / np.log2(np.arange(2, k + 2, dtype=np.float64))
    return float(np.sum(gains))


def _spearman_correlation(y_true: FloatArray, y_pred: FloatArray) -> float | None:
