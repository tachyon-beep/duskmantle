...
Set up a virtual env with `python -m venv .venv && source .venv/bin/activate`, then `pip install -r gateway/requirements.txt` for local hacking. Build the turnkey image via `docker build -t duskmantle/km:dev .`. Run it with `docker run --rm -p 8000:8000 -v $(pwd)/data:/opt/knowledge/var -v $(pwd):/workspace/repo duskmantle/km:dev`, which exposes the API at `http://localhost:8000`. Trigger a manual ingest inside the container with `docker exec <container> python -m gateway.ingest.cli rebuild --profile local`.
